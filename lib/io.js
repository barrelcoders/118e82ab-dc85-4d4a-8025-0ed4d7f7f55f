    ...
    return {
        init: function(server) {
            ...
            objServ.sockets.on('connection', function(client) {
                ...
                client.on('updatePlayerName', function(args) {
                    table = tables.getTable(args.tableId);
                    if(table){
                        var player = table.getPlayerById(args.playerId);
                        player.playerInfo.displayName = args.name;
                        client.emit('updatePlayerNameSuccess', {player: player, tableId: args.tableId});
                        client.broadcast.emit('updatePlayerNameSuccess', {player: player, tableId: args.tableId});
                    }
                });
